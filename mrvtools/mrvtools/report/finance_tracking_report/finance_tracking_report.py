...
def get_datas(filters):
	...
	if filters.get("year"):
		frappe.log_error("filter",frappe.utils.today()[0:4])
		conditions += f" AND BDS.financial_year = '{filters.get('year')}' AND  CFTB.financial_year = '{filters.get('year')}'"
	if filters.get("year") ==None:
		frappe.log_error("filter 1",frappe.utils.today()[0:4])
		conditions += f" AND BDS.financial_year = {frappe.utils.today()[0:4]}" 
	if filters.get("key_sector"):
		conditions += f" AND P.key_sector like '{filters.get('key_sector')}' AND  CFTB.financial_year = '{filters.get('year')}"
	if filters.get("key_sub_sector"):
		conditions += f" AND P.key_sub_sector like '{filters.get('key_sub_sector')}'"
	
	if filters.get("year") or filters.get("year")==None:
		query= f"""
				SELECT
